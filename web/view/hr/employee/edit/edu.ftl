        ...
        $('.icon-delete').off('click').on('click',function(){
            var uid=$(this).attr('uid');
                if(uid){
                    ...
                    if(r==true){
                        document.location.href='/hr/employeeEdu!delete.dhtml?id='+uid;
                    }
                }
        });
...
<!--右侧详细信息begin-->
        <div class="PopRight g-edit border-solid">
            ...
            <form action="/hr/employeeEdu!saveEdu.dhtml" method="POST" class="formStyle" name="editForm" id="editForm"
...
<p class="alignright mart5"><a class="button" href="/hr/employeeEdu!viewEdu.dhtml?id=${hrEmployee.id?c}" id="btnSave">完成</a></p>
<!--右侧详细信息over-->
</@common.html>
