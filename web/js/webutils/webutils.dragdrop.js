WEBUTILS.dragdrop = (function () {
    ...
    return {
        ...
        controlTd_ddmove:function (controlObj, callback) {
            if (controlObj) {
                $(controlObj).off('mousedown').on('mousedown', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    if (e.which == 1) {
                        ...
                        var oldobj = $(this), cx, cy, shadowObj,
                            lay = WEBUTILS.layout.block(),
                            shadowObj = $(String.formatmodel(ddmoveShadow, {"object":String.formatmodel(ddmoveControl, {'text':$(oldobj).prev('th').text()})})),
                            shadowW = $(oldobj).width(),
                            shadowH = $(oldobj).height(),
                            xx = e.clientX,
                            yy = e.clientY;
                        $(lay).on("mousemove", function (e) {
                            ...
                            if (xx != e.clientX || yy != e.clientY) {
                                ...
                                cy = e.clientY <= 0 ? 0 : e.clientY >= $(document).height() ? $(document).height() : e.clientY - shadowH / 2;
                                $('.md-plus').append(shadowObj);
                                cx = cx - $('.md-plus').offset().left + 40;
                                var scollOffset=WEBUTILS.dragdrop.scollPostion();
                                shadowObj.css({left:cx, top:cy-(150-scollOffset.top)});
                            }
                            WEBUTILS.dragdrop.appMove('true');
                        });
                        $(lay).on("mouseup", function (e) {
                            ...
                            if (WEBUTILS.dragdrop.appMove() == 'true') {
                                $.doTimeout('ddmove', 50, function () {
                                    var currentIn = WEBUTILS.status.currentIn(e);
                                    if (currentIn == 'controlTd') {
                                        var tdInObj = WEBUTILS.status.controlTdInObj();
                                        if (tdInObj) {
                                            var table=$(tdInObj).parents('.md-set-table');
                                            if(table){
                                                if($('td',table).size()==1){
                                                    WEBUTILS.msg.alertFail('不能和此控件位置互换!列数不匹配。');
                                                }else{
                                                    var oldTable=oldobj.parents('.md-set-table');
                                                    if(oldTable){
                                                        var oldThObj=oldobj.prev();
                                                        var oldText=$(oldThObj).text();

                                                        var oldControlType=$(oldobj).attr('controlType');
                                                        var oldAcId=$(oldobj).attr('acId');
                                                        var oldTdInner=$('.td-inner',oldobj).clone(true);


                                                        var toThObj=tdInObj.prev();
                                                        var toText=$(toThObj).text();

                                                        var toControlType=$(tdInObj).attr('controlType');
                                                        var toAcId=$(tdInObj).attr('acId');
                                                        var toTdInner=$('.td-inner',tdInObj).clone(true);


                                                        $(oldThObj).text(toText);

                                                        $(oldobj).attr('controlType',toControlType);
                                                        $(oldobj).attr('acId',toAcId);
                                                        $('.td-inner',oldobj).remove();
                                                        $(oldobj).append(toTdInner);


                                                        $(toThObj).text(oldText);

                                                        $(tdInObj).attr('controlType',oldControlType);
                                                        $(tdInObj).attr('acId',oldAcId);
                                                        $('.td-inner',tdInObj).remove();
                                                        $(tdInObj).append(oldTdInner);
                                                        callback(oldobj,tdInObj);
                                                    }
                                                }
                                            }
                                        }
