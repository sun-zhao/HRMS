WEBUTILS.validator = (function () {
    ...
    var refresh = function (result, elem, mode, msg) {
        ...
        if (!result) {
            var mobile=false;
            if($(elem).parents('.row-fluid').size()==1){
                mobile=true;
            }
            if(mobile){
                var inputDiv=$(elem).parents('.row-fluid');
                if(inputDiv){
                    inputDiv.removeClass('success');
                    inputDiv.removeClass('warning');
                    inputDiv.removeClass('error');
                    inputDiv.removeClass('info');
                    inputDiv.addClass('error');
                    $(elem).next('.help-inline').text(msg);
                }
            }else{
                var dateFlag=false;
                var dateDiv=$(elem).parent();
                if(dateDiv){
                    if($(dateDiv).hasClass('md-clander')){
                        dateFlag=true;
                    }
                }
                var inputDiv=false;
                if(dateFlag){
                    inputDiv=$(elem).parent().parent('.has-general');
                }else{
                    inputDiv=$(elem).parent('.has-general');
                }
                if(inputDiv){
                    inputDiv.removeClass('has-success');
                    inputDiv.removeClass('has-warning');
                    inputDiv.removeClass('has-error');
                    inputDiv.addClass('has-error');

                    $('.control-label',inputDiv).text(msg);
                    $('.control-label',inputDiv).width($(elem).width()-5);
                    $('.control-label',inputDiv).height($(elem).height()-10);
                    $('.control-label',inputDiv).show();
                    $('.control-label',inputDiv).off('click').on('click',function(){
                        $(this).hide();
                    });
