package hrms.mingdao.common.service;
...
public class TempAttService extends HQuery {
    ...
    @Transactional(type = TransactionType.READ_ONLY)
    public List<TempAtt> getListByTokenId(String attKey, String tokenId,String companyId,String userId) {
        return $($eq("companyId",companyId),$eq("userId",userId),
                $eq("attKey", attKey), $eq("tokenId", tokenId), $eq("useYn", "Y")).list(TempAtt.class);
    }

    @Transactional(type = TransactionType.READ_ONLY)
    public List<TempAtt> getListByNotTokenId(String attKey, String tokenId,String companyId,String userId) {
        return $($eq("companyId",companyId),$eq("userId",userId),
